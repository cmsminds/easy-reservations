...
class Reservation_Cancellation_Request_Deleted_Email extends WC_Email {
	...
	public function __construct() {
		...
		$this->id          = 'reservation_cancellation_request_deleted_email';
		$this->title       = __( 'Easy Reservations: Reservation Cancellation Request Deleted Email', 'easy-reservations' );
		$this->description = __( 'An email sent to the customer when admin deletes reservation cancellation request.', 'easy-reservations' );

		// For admin area to let the user know we are sending this email to the customer.
		$this->customer_email = true;
		$this->heading        = __( 'Reservation Cancellation Request Deleted', 'easy-reservations' );

		// translators: placeholder is {blogname}, a variable that will be substituted when email is sent out.
		$this->subject = sprintf( _x( '[%s] Reservation Cancellation Request Deleted', 'default email subject for reservation cancellation request approved being sent to the customer', 'easy-reservations' ), '{blogname}' );

		// Template paths.
		$this->template_html  = 'reservation-cancellation-request-deleted-html.php';
		$this->template_plain = 'plain/reservation-cancellation-request-deleted-plain.php';

		add_action( 'ersrv_send_reservation_cancellation_request_deleted_notification', array( $this, 'ersrv_ersrv_send_reservation_cancellation_request_deleted_notification_callback' ) );
	 ...
	 * @since 1.0.0
	 */
	public function ersrv_ersrv_send_reservation_cancellation_request_deleted_notification_callback( $line_item_id ) {
